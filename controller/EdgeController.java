...
public class EdgeController {
	...
	public void updateView(Graphics2D g, Class typeOfGraph) {
		...
		int adjacent = (int) Math
				.abs(model.getSource().getLocation().getX() - model.getDestination().getLocation().getX());
		int opposite = (int) Math
				.abs(model.getSource().getLocation().getY() - model.getDestination().getLocation().getY());
